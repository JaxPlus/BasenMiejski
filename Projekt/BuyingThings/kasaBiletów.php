...
<body>
    ...
    <main class="mainThanks" id="thanksMessageCut" class="blur">
        <div class="thanksMessage">
            <form action="../konto.php" method="post">
                <?php

                    $buying = new BuyingTickets();
                    $connection = new ConnectToDB();
                    $connection = $connection->getConnection();

                    // Ustawia wartości przyjścia jak i 
                    for ($i = 1; $i <= $_SESSION['numberOfTickets']; $i++)
                    { 
                        $buying->setTickets($i, $_POST['arrivalDate'. $i]);
                        $buying->setReducedTickets($i, @$_POST['kid'. $i]);
                        $buying->toCheckDate($i);
                    }
                ...
                ?>
                <input type="submit" class="submitButtons" value="Kup">
            </form>
        </div>
    </main>
